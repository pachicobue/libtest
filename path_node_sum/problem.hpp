...
namespace libtest {
struct path_node_sum
{
    static constexpr const char* path       = "path_node_sum";
    static constexpr const char* name       = "Path of Tree (Node Weight Sum)";
    ...
    static void generate_input(std::ofstream& input_file)
    {
        ...
        pr.println(v, q);
        const auto a = rng.gen_vec(v, a_min, a_max);
        pr.println(a);
    ...
    static void generate_output(std::ifstream& input_file, std::ofstream& output_file)
    {
        ...
        for (usize i = 0; i < q; i++) {
            ...
            while (true) {
                ...
                while (depth[u] < depth[v]) { ans += a[v], v = par[v]; }
                if (u == v) {
                    break;
                } else {
                    ans += a[u], u = par[u];
    ...
    static bool judge(std::ifstream& input_file, std::ifstream& generated_output_file, std::ifstream& solution_output_file)
    {
        ...
        for (usize i = 0; i < q; i++) {
            const auto gen_a = gen_sc.read<ll>();
            const auto sol_a = sol_sc.may_read<ll>();
    ...
    struct small_constraints
    {
        ...
        static constexpr ll a_min = -100, a_max = 100;
