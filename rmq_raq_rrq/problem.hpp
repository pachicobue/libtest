...
struct rmq_raq_rrq
{
    static constexpr const char* path       = "rmq_raq_rrq";
    static constexpr const char* name       = "Range Min Query & Range Add Query & Range Rotate Query";
    ...
    static void generate_input(std::ofstream& input_file)
    {
        ...
        pr.println(vs);
        for (usize i = 0; i < q; i++) {
            const auto type = rng.gen(0UL, 5UL);
            if (type == 0) {
    ...
    static void generate_output(std::ifstream& input_file, std::ofstream& output_file)
    {
        ...
        for (usize i = 0; i < q; i++) {
            ...
            if (type == 0) {
                ...
                const auto [l, r] = sc.read_vals<usize, usize>();
                const auto h      = vs[l];
                for (usize i = l; i + 1 < r; i++) { vs[i] = vs[i + 1]; }
                vs[r - 1] = h;
            } else {
                const auto [l, r] = sc.read_vals<usize, usize>();
                const auto b      = vs[r - 1];
                for (usize i = r - 1; i >= l + 1; i--) { vs[i] = vs[i - 1]; }
                vs[l] = b;
            }
        }
    }
